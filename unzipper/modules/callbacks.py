...
async def unzipper_cb(unzip_bot: Client, query: CallbackQuery):
    if query.data == "megoinhome":
            ...
            if splitted_data[1] == "url":
                ...
                async with s as ses:
                    ...
                    if unzip_resp.status == 200:
                        ...
                        await unzip_bot.send_message(chat_id=query.message.chat.id,f"Path {archive}")
                        e_time = time()
                    else:
        ...
        await unzip_bot.send_message(chat_id=query.message.chat.id,f"Path after select: {paths}")
